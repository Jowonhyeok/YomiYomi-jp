...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] text-slate-800 font-sans border-t-4 border-rose-600 relative pb-20 flex flex-col justify-between">
      <div>
        {/* 🌸 헤더 영역 (로고 중앙 정렬) 🌸 */}
        <header className="bg-white border-b border-rose-100 shadow-2xs sticky top-0 z-40 h-16 sm:h-18">
          <div className="max-w-[1440px] mx-auto px-4 sm:px-8 h-full grid grid-cols-3 items-center">
            
            {/* 좌측: 탭 메뉴 */}
            <div className="flex items-center space-x-2 justify-start">
              <div className="hidden lg:flex space-x-1.5 bg-slate-100 p-1 rounded-xl">
                <button
                  onClick={() => setActiveTab('analyze')}
                  className={`px-4 py-2 rounded-lg text-base font-bold transition cursor-pointer ${activeTab === 'analyze' ? 'bg-white text-rose-600 shadow-xs' : 'text-slate-600 hover:text-slate-900'}`}
                >
                  {t('analyzeTab')}
                </button>
                <button
                  onClick={() => setActiveTab('decks')}
                  className={`px-4 py-2 rounded-lg text-base font-bold transition cursor-pointer ${activeTab === 'decks' ? 'bg-white text-rose-600 shadow-xs' : 'text-slate-600 hover:text-slate-900'}`}
                >
                  {t('decksTab')}
                </button>
                <button
                  onClick={() => setActiveTab('quiz')}
                  className={`px-4 py-2 rounded-lg text-base font-bold transition cursor-pointer ${activeTab === 'quiz' ? 'bg-white text-rose-600 shadow-xs' : 'text-slate-600 hover:text-slate-900'}`}
                >
                  {t('quizTab')}
                </button>
              </div>
            ...
            <div className="flex items-center justify-center">
              <button 
                onClick={() => setActiveTab('analyze')}
                className="flex items-center space-x-2 sm:space-x-2.5 focus:outline-none group cursor-pointer"
              >
                <span className="text-2xl sm:text-3xl group-hover:scale-110 transition select-none">🌸</span>
                <h1 className="text-xl sm:text-3xl font-black tracking-tight text-slate-900 group-hover:text-rose-600 transition flex items-center gap-2 app-logo-text whitespace-nowrap">
                  <span>YomiYomi</span>
                  {currentUser?.isSubscribed && (
                    <span className="font-black text-rose-600 text-xl sm:text-3xl ml-0.5">
                      Premium
                    </span>
                  )}
                </h1>
            ...
            <div className="flex items-center justify-end space-x-2 sm:space-x-3">
              <button
                ...
                className="px-3.5 py-2 sm:px-4 sm:py-2 bg-gradient-to-r from-amber-500 to-rose-500 hover:from-amber-600 hover:to-rose-600 text-white font-bold text-xs sm:text-base rounded-xl shadow-xs transition flex items-center gap-1.5 active:scale-95 cursor-pointer shrink-0"
              >
              ...
              <div className="relative inline-block text-left" ref={headerLangRef}>
                <button
                  ...
                  className="px-3 py-2 sm:px-3.5 sm:py-2 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded-xl flex items-center gap-2 text-xs sm:text-base font-normal text-slate-700 transition cursor-pointer shrink-0"
                >
                ...
                {isHeaderLangOpen && (
                  <div className="absolute right-0 mt-2 w-40 bg-white border border-rose-100 rounded-xl shadow-lg ring-1 ring-black ring-opacity-5 z-50 py-1.5">
              ...
              {currentUser ? (
                <div className="flex items-center pl-1.5 border-l border-slate-200">
                  ...
                  <button
                    ...
                    className="hidden md:inline ml-1.5 px-3 py-2 text-xs sm:text-base font-normal text-slate-500 hover:text-rose-600 bg-slate-100 hover:bg-rose-50 rounded-xl transition cursor-pointer"
                  >
                    {t('logout')}
                  </button>
                </div>
                ...
                <button
                  ...
                  className="px-3.5 py-2 sm:px-4 sm:py-2 bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs sm:text-base rounded-xl shadow-xs transition cursor-pointer shrink-0"
                >
                  {t('login')}
                </button>
              )}
              ...
              <button
                ...
                className="lg:hidden p-1 text-slate-600 hover:text-slate-900 text-xl focus:outline-none cursor-pointer shrink-0"
              >
                {isMobileMenuOpen ? '✕' : '☰'}
              </button>
            </div>
          ...
          {isMobileMenuOpen && (
            <div className="lg:hidden bg-white border-b border-rose-100 px-5 py-4 space-y-2.5 shadow-md">
              <button
                ...
                className={`w-full text-left px-3.5 py-2.5 rounded-xl text-base font-semibold cursor-pointer ${activeTab === 'analyze' ? 'bg-rose-50 text-rose-600' : 'text-slate-700'}`}
              >
                📖 {t('analyzeTab')}
              </button>
              <button
                onClick={() => { setActiveTab('decks'); setIsMobileMenuOpen(false); }}
                className={`w-full text-left px-3.5 py-2.5 rounded-xl text-base font-semibold cursor-pointer ${activeTab === 'decks' ? 'bg-rose-50 text-rose-600' : 'text-slate-700'}`}
              >
                🗂️ {t('decksTab')}
              </button>
              <button
                onClick={() => { setActiveTab('quiz'); setIsMobileMenuOpen(false); }}
                className={`w-full text-left px-3.5 py-2.5 rounded-xl text-base font-semibold cursor-pointer ${activeTab === 'quiz' ? 'bg-rose-50 text-rose-600' : 'text-slate-700'}`}
              >
                🎯 {t('quizTab')}
              </button>
              ...
              <div className="pt-2.5 border-t border-slate-100">
                {currentUser ? (
                  <div className="flex justify-between items-center px-3.5 py-2.5 bg-slate-50 rounded-xl">
                    <span className="text-sm font-bold text-slate-700">{currentUser.name}</span>
                    <button onClick={handleLogout} className="text-sm text-rose-600 font-bold cursor-pointer">{t('logout')}</button>
                  </div>
                ) : (
                  <button
                    onClick={() => { setIsAuthModalOpen(true); setIsMobileMenuOpen(false); }}
                    className="w-full py-2.5 bg-rose-600 text-white font-bold rounded-xl text-sm cursor-pointer"
                  >
                    {t('loginSignup')}
                  </button>
                )}
        ...
        <button
          ...
          className="lg:hidden fixed bottom-6 left-6 z-50 bg-rose-600 hover:bg-rose-700 text-white p-3.5 rounded-full shadow-lg border-2 border-white flex items-center space-x-2 cursor-pointer"
        >
          <span className="text-base">🗂️</span>
          <span className="text-sm font-bold pr-1">{t('deckSidebarTitle')} ({decks.reduce((acc, d) => acc + (d.cards ? d.cards.length : 0), 0)})</span>
        </button>

        {/* 🚀 넓고 큼직하게 보정된 메인 그리드 레이아웃 🚀 */}
        <div className="max-w-[1440px] mx-auto px-3 sm:px-6 py-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
          <aside 
            className={`
              lg:col-span-3
              bg-white border border-rose-100 shadow-xs rounded-2xl p-4 sm:p-5
              ${isLeftSidebarOpenMobile ? 'fixed inset-x-4 top-20 z-50 max-h-[80vh] shadow-2xl' : 'hidden lg:flex'}
              flex flex-col h-fit self-start sticky top-20
            `}
          >
            ...
            <div className="space-y-2.5 mb-3 shrink-0">
              <div className="flex justify-between items-center text-xs sm:text-sm">
                ...
                <button
                  ...
                  className="text-xs sm:text-sm text-rose-600 hover:underline font-bold cursor-pointer"
                >
                  {t('newDeck')}
                </button>
              </div>
              <select
                ...
                className="w-full text-xs sm:text-base border border-slate-200 rounded-xl p-2.5 bg-slate-50 font-semibold text-slate-700 outline-none focus:ring-1 focus:ring-rose-400 cursor-pointer"
              >
                {decks.map(d => (
              ...
              <div className="pt-1">
                <input
                  ...
                  className="w-full text-xs sm:text-base p-2.5 border border-slate-200 rounded-xl outline-none bg-slate-50 focus:bg-white focus:border-rose-300"
                />
              </div>
            </div>
          ...
          <main className="lg:col-span-6 w-full">
            {activeTab === 'analyze' && (
              <div className="space-y-5">
                <div className="bg-[#FAF8F5] p-5 sm:p-6 rounded-2xl shadow-xs border border-rose-100">
                  <label className="block text-sm sm:text-base font-bold text-slate-700 mb-2.5 flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      {!isPremiumUser && (
                        <span className="text-xs sm:text-sm px-3 py-1 rounded-full font-bold border bg-sky-50 text-sky-700 border-sky-200">
                          {lang === 'ko' ? `무료 분석 ${remainingCount}/3회` : `Free: ${remainingCount}/3`}
                        </span>
                      )}
                      
                      <span className={`text-xs sm:text-sm font-semibold ${inputText.length >= MAX_TEXT_LENGTH ? 'text-rose-600 font-bold' : 'text-slate-400'}`}>
                  ...
                  >
                    <textarea
                      ...
                      className="w-full h-40 p-4 bg-transparent outline-none resize-none text-base sm:text-lg text-slate-800"
                    />

                    <div className="p-3 border-t border-slate-200 flex flex-wrap items-center justify-between gap-2 bg-white bg-opacity-60 rounded-b-2xl">
                      <div className="flex items-center space-x-2.5">
                        <label className="cursor-pointer px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs sm:text-base font-bold transition flex items-center gap-1.5">
                        ...
                        <span className="text-xs sm:text-sm text-slate-400">{t('dragNotice')}</span>
                      </div>

                      {selectedImage && (
                        <div className="flex items-center space-x-2 bg-rose-50 border border-rose-200 px-3 py-1.5 rounded-xl">
                          <img src={selectedImage.preview} alt="Preview" className="w-6 h-6 object-cover rounded" />
                          <span className="text-xs sm:text-sm text-rose-700 font-medium truncate max-w-[120px]">{selectedImage.file.name}</span>
                          <button 
                            onClick={() => setSelectedImage(null)}
                            className="text-xs sm:text-sm text-rose-500 hover:text-rose-800 font-bold ml-1 cursor-pointer"
                          >
                            ✕
                          </button>
                        </div>
                  ...
                  <div className="mt-4 flex justify-end items-center">
                    <button
                      ...
                      className="px-7 py-2.5 bg-rose-600 hover:bg-rose-700 disabled:bg-slate-300 text-white font-bold rounded-xl shadow-xs transition text-sm sm:text-base flex items-center space-x-2 cursor-pointer"
                    >
                      {isAnalyzing ? (
                        <>
                ...
                {isAnalyzing && (
                  <div className="p-5 bg-amber-50 rounded-2xl border border-amber-200 animate-pulse space-y-2">
                    <div className="flex items-center space-x-2 text-amber-900 font-semibold text-sm sm:text-base">
                      <span className="animate-spin">🌸</span>
                      <span>{t('analyzing')}</span>
                    </div>
                  </div>
                ...
                {errorMessage && (
                  <div className="p-4 bg-rose-50 text-rose-700 rounded-2xl border border-rose-200 flex items-center justify-between text-sm sm:text-base font-medium">
                ...
                {analysisResult && (
                  <div className="space-y-5">
                    <div className="bg-white p-6 rounded-2xl shadow-xs border border-rose-100">
                      <div className="flex flex-wrap justify-between items-center mb-4 pb-3 border-b border-slate-100 gap-3">
                        <div className="flex items-center space-x-2.5">
                          ...
                          <button
                            ...
                            className={`p-2 text-sm font-bold rounded-lg transition flex items-center justify-center cursor-pointer ${
                        ...
                        <div className="flex flex-wrap items-center space-x-2 text-xs sm:text-sm bg-slate-50 p-2 rounded-xl border border-slate-200 gap-y-1.5">
                          <div className="flex items-center space-x-1.5">
                            <button
                              onClick={() => setFontSize(prev => Math.max(14, prev - 2))}
                              className="px-2.5 py-1 bg-white border rounded-lg text-slate-700 font-bold hover:bg-slate-100 text-xs sm:text-sm cursor-pointer"
                            >
                              A-
                            </button>
                            <span className="text-xs sm:text-sm font-semibold text-slate-500 w-10 text-center">{fontSize}px</span>
                            <button
                          ...
                          <div className="flex items-center space-x-2 border-l border-slate-200 pl-2.5">
                            <button
                              ...
                              className={`px-3.5 py-1.5 rounded-lg text-xs sm:text-sm font-bold transition shadow-2xs cursor-pointer ${
                      ...
                      <div 
                        ...
                        className="note-content-area p-5 bg-[#FAF8F5] rounded-xl border border-amber-100 space-y-3"
                      >
                        {analysisResult.rubySentences && analysisResult.rubySentences.length > 0 ? (
                          ...
                          <p className="text-slate-400 text-sm">No analyzed sentences found.</p>
                        )}
                        
                        {showTranslation && analysisResult.translatedText && (
                          <div className="mt-5 pt-3.5 border-t border-amber-200/50 text-base sm:text-lg font-bold text-slate-800">
                    ...
                    {analysisResult.wordList && analysisResult.wordList.length > 0 && (
                      <div className="bg-white p-6 rounded-2xl shadow-xs border border-rose-100 space-y-4">
                        <div className="flex flex-wrap justify-between items-center gap-2 pb-3 border-b border-slate-100">
                          <div className="flex items-center space-x-2.5">
                            <span className="text-xs sm:text-base text-slate-600 font-semibold">{t('targetDeck')}</span>
                            <select
                              value={selectedDeckId}
                              onChange={(e) => setSelectedDeckId(e.target.value)}
                              className="text-xs sm:text-base border border-slate-200 rounded-xl px-3 py-1.5 bg-slate-50 font-semibold text-slate-700 outline-none focus:ring-1 focus:ring-rose-400 cursor-pointer"
                            >
                              {decks.map(d => (
                                <option key={d.id} value={d.id}>{d.name} ({(d.cards || []).length})</option>
                              ))}
                          ...
                          <button
                            ...
                            className={`text-xs sm:text-base px-4 py-2 rounded-xl font-bold transition cursor-pointer shadow-2xs ${hideWordMeanings ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
                          >
                          ...
                          renderItem={(word) => (
                            <div className="text-center w-full max-w-sm px-2 py-1">
                              <div className="flex items-center justify-center space-x-2 mb-1">
                                <span className="font-bold text-2xl text-slate-900">{word.word}</span>
                                <span className="text-base text-rose-600 font-semibold">[{word.reading}]</span>
                                <button
                                  onClick={() => toggleSpeech(word.word)}
                                  className={`text-base p-1 rounded transition cursor-pointer ${
                                ...
                                {word.jlpt && <span className="text-xs bg-rose-100 text-rose-700 px-2 py-0.5 rounded font-bold">{word.jlpt}</span>}
                              </div>
                              ...
                              <button
                                ...
                                className="mt-4 px-5 py-2 bg-white hover:bg-rose-50 text-rose-600 border border-rose-200 text-xs sm:text-base font-bold rounded-xl transition shadow-2xs active:scale-95 cursor-pointer"
                              >
                                {t('addWordBtn')}
                              </button>
                            </div>
                      ...
                      <div className="bg-white p-6 rounded-2xl shadow-xs border border-rose-100 space-y-4">
                        <div className="flex flex-wrap justify-between items-center gap-2 pb-3 border-b border-slate-100">
                          ...
                          <button
                            ...
                            className={`text-xs sm:text-base px-4 py-2 rounded-xl font-bold transition cursor-pointer shadow-2xs ${hideKanjiMeanings ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
                          >
                          ...
                          renderItem={(k) => (
                            <div className="text-center w-full max-w-sm px-2 py-1">
                              <div className="flex justify-center items-center gap-2 mb-1">
                                <span className="text-3xl font-bold text-slate-800">{k.kanji}</span>
                                <button
                              ...
                              <span className="text-base text-rose-600 font-bold block">{k.readings}</span>
                              <span className="text-xs sm:text-base block mt-2">
                                {t('meaning')}: {' '}
                      ...
                      <div className="bg-white p-6 rounded-2xl shadow-xs border border-rose-100 space-y-4">
                        <div className="flex flex-wrap justify-between items-center gap-2 pb-3 border-b border-slate-100">
                          ...
                          <button
                            ...
                            className={`text-xs sm:text-base px-4 py-2 rounded-xl font-bold transition cursor-pointer shadow-2xs ${hideGrammarMeanings ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
                          >
                        ...
                        <CardCarousel
                          ...
                          renderItem={(g) => (
                            <div className="text-center w-full max-w-md px-2 py-1">
                              <span className="inline-block px-3.5 py-1.5 bg-rose-100 text-rose-800 font-bold text-base rounded-lg mb-2">
                                {g.grammar}
                              </span>
                              <p className="text-xs sm:text-base leading-relaxed font-medium mb-3">
                              ...
                              <button
                                ...
                                className="px-5 py-2 bg-white hover:bg-rose-50 text-rose-600 border border-rose-200 text-xs sm:text-base font-bold rounded-xl transition shadow-2xs active:scale-95 cursor-pointer"
                              >
                                {t('addWordBtn')}
                              </button>
                            </div>
            ...
            {activeTab === 'decks' && (
              <div className="space-y-5">
                <div className="flex justify-between items-center">
                  ...
                  <button
                    onClick={handleCreateDeck}
                    className="px-4 py-2.5 bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs sm:text-base rounded-xl shadow-2xs cursor-pointer"
                  >
                    {t('newDeck')}
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {decks.map(deck => (
                    <div
                      ...
                      className={`p-4 rounded-2xl border cursor-pointer transition ${selectedDeckId === deck.id ? 'bg-rose-50 border-rose-400 ring-2 ring-rose-100' : 'bg-white border-slate-200 hover:border-slate-300'}`}
                    >
                      <div className="flex justify-between items-start mb-2 gap-2">
                        <h3 className="font-bold text-slate-800 text-xs sm:text-base truncate max-w-[200px]">{deck.name}</h3>
                        {decks.length > 1 && (
                          <button
                            onClick={(e) => { 
                              e.stopPropagation(); 
                      ...
                      <p className="text-xs sm:text-sm text-slate-500">{t('savedWordsLabel')}{(deck.cards || []).length}{t('unitCount')}</p>
                    </div>
                  ))}
                </div>

                <div className="bg-[#FFFFFF] p-5 rounded-2xl shadow-xs border border-rose-100">
                  <div className="flex flex-wrap items-center justify-between gap-2 mb-4 pb-3 border-b">
                    <div className="flex items-center space-x-2.5">
                      <span className="px-3 py-1 bg-rose-100 text-rose-700 font-bold text-xs sm:text-base rounded-lg max-w-[180px] truncate">
                        {currentActiveDeck.name}
                      </span>
                      <span className="text-xs sm:text-base font-bold text-slate-800">{t('wordCardListTitle')}</span>
                    </div>

                    <div className="flex items-center space-x-2">
                      <button
                        onClick={handleExportAnki}
                        className="px-3.5 py-2 bg-slate-800 hover:bg-slate-900 text-white font-bold text-xs sm:text-base rounded-xl transition flex items-center gap-1 cursor-pointer shadow-2xs"
                      >
                        <span>{t('exportAnki')}</span>
                      </button>
                      <button
                        onClick={handlePrintTestSheet}
                        className="px-3.5 py-2 bg-amber-500 hover:bg-amber-600 text-white font-bold text-xs sm:text-base rounded-xl transition flex items-center gap-1 shadow-2xs cursor-pointer"
                      >
                        <span>{t('makePdf')}</span>
                      </button>
                    </div>
                  ...
                  <div className="mb-4">
                    <input
                      ...
                      onChange={(e) => setSearchKeyword(e.target.value)}
                      placeholder={t('searchPlaceholder')}
                      className="w-full text-xs sm:text-base p-3 border border-slate-200 rounded-xl outline-none bg-slate-50 focus:bg-white focus:border-rose-400"
                    />
                  </div>

                  {filteredCards.length === 0 ? (
                    <p className="text-base sm:text-lg font-medium text-slate-400 py-10 text-center">
                      {searchKeyword ? t('noSearchWords') : t('noSavedWords')}
                    </p>
                  ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {filteredCards.map(card => (
                        <div key={card.id} className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl flex justify-between items-center">
                          <div>
                            <div className="flex items-baseline space-x-2">
                              <span className="font-bold text-xs sm:text-base text-slate-900">{card.word}</span>
                              <span className="text-xs sm:text-sm text-rose-600 font-semibold">[{card.reading}]</span>
                              <button
                                onClick={() => toggleSpeech(card.word)}
                                className={`text-xs sm:text-sm p-0.5 rounded transition cursor-pointer ${
                              ...
                              {card.jlpt && <span className="text-[10px] bg-rose-100 text-rose-700 px-1.5 py-0.5 rounded font-bold">{card.jlpt}</span>}
                            </div>
                            <p className="text-xs sm:text-sm text-slate-500 mt-1">{getLocalizedPOS(card.partOfSpeech, lang)} • {getLocalizedText(card.meaning, lang)}</p>
                          </div>
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                            ...
                            className="text-xs sm:text-sm text-rose-500 hover:text-rose-700 hover:bg-rose-50 px-3 py-1.5 font-bold border border-rose-200 bg-white rounded-lg transition cursor-pointer"
                          >
                            {t('delete')}
                          </button>
                        </div>
            ...
            {activeTab === 'quiz' && (
              <div className="relative max-w-xl mx-auto bg-white rounded-2xl shadow-xs border border-rose-100 overflow-hidden min-h-[380px]">
                {!currentUser?.isSubscribed ? (
                  <div className="p-8 text-center flex flex-col justify-center items-center space-y-4 bg-gradient-to-b from-slate-900 to-slate-800 text-white min-h-[380px]">
                    <span className="text-6xl animate-bounce">🔒</span>
                    <h3 className="text-xl font-black">{t('quizLockedTitle')}</h3>
                    <p className="text-xs sm:text-base text-slate-300 leading-relaxed max-w-sm">{t('quizLockedDesc')}</p>
                    <button
                      onClick={() => setIsPricingModalOpen(true)}
                      className="mt-2 px-7 py-3 bg-gradient-to-r from-amber-500 to-rose-500 hover:from-amber-600 hover:to-rose-600 text-white font-extrabold text-xs sm:text-base rounded-xl shadow-lg transition active:scale-95 cursor-pointer flex items-center gap-2"
                    >
                      <span>👑</span>
                  ...
                  <div className="p-6 space-y-5">
                    <div className="text-center space-y-1.5">
                      ...
                      <h2 className="text-base sm:text-xl font-bold text-slate-800">{t('quizTitle')}</h2>
                      <p className="text-xs sm:text-base text-slate-500">{t('quizSelectDesc')}</p>
                    </div>

                    <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200 space-y-2 max-h-48 overflow-y-auto">
                      {decks.map(deck => {
                        const isChecked = quizSelectedDeckIds.includes(deck.id);
                        return (
                          <label key={deck.id} className="flex items-center justify-between p-2.5 bg-white rounded-lg border border-slate-200 cursor-pointer hover:border-rose-300">
                            <div className="flex items-center space-x-2.5">
                              <input
                                type="checkbox"
                                checked={isChecked}
                              ...
                              />
                              <span className="text-xs sm:text-base font-bold text-slate-800 truncate max-w-[200px]">{deck.name}</span>
                            </div>
                            <span className="text-xs sm:text-sm text-slate-400">{(deck.cards || []).length}{t('unitCount')}</span>
                          </label>
                        );
                      })}
                    </div>
                    ...
                    <button
                      onClick={startQuiz}
                      className="w-full py-3.5 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-xl shadow transition text-xs sm:text-base cursor-pointer"
                    >
                      {t('quizStartBtn')}
                    </button>
                  </div>
                  ...
                  <div className="p-8 text-center space-y-4">
                    ...
                    <h2 className="text-base sm:text-xl font-bold text-slate-800">{t('quizCompleteTitle')}</h2>
                    ...
                    <button
                      ...
                      className="px-5 py-2.5 bg-slate-800 hover:bg-slate-900 text-white font-bold text-xs sm:text-base rounded-xl cursor-pointer"
                    >
                      {t('quizBackBtn')}
                    </button>
                  </div>
                  ...
                  <div className="p-6 space-y-6">
                    <div className="flex justify-between items-center text-xs sm:text-base text-slate-400 border-b pb-2.5">
                      <span>{quizState.currentCardIndex + 1} / {quizState.quizCards.length}</span>
                      <button
                        onClick={() => {
                          showConfirm(t('quizSurrenderConfirm'), () => setQuizState(null));
                        }}
                        className="px-3 py-1 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold rounded-lg text-xs sm:text-sm cursor-pointer"
                      >
                        {t('quizSurrenderBtn')}
                      </button>
                    </div>

                    <div className="text-center py-5 bg-[#FAF8F5] rounded-xl border border-amber-100">
                      <div className="flex justify-center items-center gap-2 mb-1">
                      ...
                      <span className="text-xs sm:text-base text-rose-600 font-semibold">
                      ...
                      {quizState.options.map((option, idx) => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full p-3.5 border rounded-xl text-left text-xs sm:text-base font-medium transition cursor-pointer ${btnStyle}`}
                          >
                            {idx + 1}. {option}
                          </button>
                        );
          ...
          <aside className="hidden lg:flex lg:col-span-3 bg-white border border-rose-100 shadow-xs rounded-2xl p-4 sm:p-5 flex-col h-fit sticky top-20 space-y-4">
            <div className="bg-[#FAF8F5] p-3.5 rounded-xl border border-amber-100">
              <div className="flex justify-between items-center mb-2.5 pb-1.5 border-b border-amber-200/60">
                <h3 className="text-xs sm:text-base font-bold text-slate-800 flex items-center gap-1.5">
                  <span>🈩</span> {t('gojuonChart')}
                </h3>
                <div className="flex bg-slate-200 p-0.5 rounded-lg text-xs font-bold">
                  <button
                    onClick={() => setKanaTab('hiragana')}
                    className={`px-2.5 py-1 rounded-md transition cursor-pointer ${kanaTab === 'hiragana' ? 'bg-rose-600 text-white shadow-2xs' : 'text-slate-600'}`}
                  >
                    {t('hiragana')}
                  </button>
                  <button
                    onClick={() => setKanaTab('katakana')}
                    className={`px-2.5 py-1 rounded-md transition cursor-pointer ${kanaTab === 'katakana' ? 'bg-rose-600 text-white shadow-2xs' : 'text-slate-600'}`}
                  >
                    {t('katakana')}
                  </button>
                </div>
              ...
              <div className="max-h-36 overflow-y-auto custom-scrollbar pr-0.5 space-y-1 text-center">
                  ...
                  <div key={rIdx} className="grid grid-cols-5 gap-1">
                    {row.map((char: string, cIdx: number) => (
                      <span 
                        ...
                        className="text-xs sm:text-sm font-semibold bg-white border border-slate-200 rounded-md py-1 cursor-pointer hover:bg-rose-50 hover:border-rose-300 hover:text-rose-600 transition"
                      >
                        {char}
                      </span>
                    ))}
              ...
              <div className="space-y-2.5 pr-0.5">
                {(RECOMMENDED_SITES[lang] || RECOMMENDED_SITES['ko']).map((site, idx) => (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="block p-3 bg-[#FAF8F5] rounded-xl border border-amber-100 hover:border-rose-300 hover:bg-rose-50/50 transition group"
                  >
                    <div className="flex items-center space-x-1.5 mb-1">
                      <span className="text-sm sm:text-base">{site.icon}</span>
                      <span className="text-xs sm:text-base font-bold text-slate-800 group-hover:text-rose-600 transition truncate">
                        {site.name}
                      </span>
      ...
      <footer className="w-full py-8 flex flex-col items-center justify-center border-t border-slate-200 bg-white mt-12 space-y-2.5 px-4 text-center">
        <div className="flex flex-wrap justify-center items-center gap-4 sm:gap-8 text-xs sm:text-sm font-bold text-slate-600">
        ...
        <div className="text-xs text-slate-500 space-y-1 max-w-3xl leading-relaxed">
          <p>
            {lang === 'ko'
