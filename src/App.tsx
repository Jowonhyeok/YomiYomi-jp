...
export default function App() {
  ...
  const handleExportAnki = () => {
    ...
    link.href = url;
    link.setAttribute('download', `${currentDeck.name}_Anki_Deck.txt`);
                    ...
                    <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl text-left my-2 space-y-1">
                      <label className="flex items-start space-x-2 text-xs text-slate-700 cursor-pointer">
                        ...
                        <span className="leading-tight text-[11px]">
                          ...
                          {lang === 'ja' && (
                            <>[必須] 自動更新および<button type="button" onClick={() => openLegalDoc('refund')} className="text-rose-600 underline font-bold">返金・解約ポリシー</button>에 동의합니다.</>
                          )}
                        </span>
                      </label>
                    </div>
